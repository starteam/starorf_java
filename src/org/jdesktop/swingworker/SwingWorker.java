/* 
 * $Id: SwingWorker.java,v 1.2 2009/02/25 04:10:50 ceraj Exp $
 * 
 ...
 * <i>Event Dispatch Thread</i> (EDT) for all GUI-related activities. This involves inter-thread communication which can be tricky to implement.
 * <p>
 * {@code SwingWorker} is designed for situations where you need to have a long running task run in a background thread and provide updates to the UI either
 * when done, or while processing. Subclasses of {@code SwingWorker} must implement the {@see #doInBackground} method to perform the background computation.
 * <p>
 * <b>Workflow</b>
 * <p>
 ...
 * <li>
 * <p>
 * <i>Current</i> thread: The {@link #execute} method is called on this thread. It schedules {@code SwingWorker} for the execution on a <i>worker</i> thread and
 * returns immediately. One can wait for the {@code SwingWorker} to complete using the {@link #get get} methods.
 * <li>
 * <p>
 * <i>Worker</i> thread: The {@link #doInBackground} method is called on this thread. This is where all background activities should happen. To notify {@code
 * PropertyChangeListeners} about bound properties changes use the {@link #firePropertyChange firePropertyChange} and {@link #getPropertyChangeSupport} methods.
 * By default there are two bound properties available: {@code state} and {@code progress}.
 * <li>
 * <p>
 * <i>Event Dispatch Thread</i>: All Swing related activities occur on this thread. {@code SwingWorker} invokes the {@link #process process} and {@link #done}
 ...
 * <p>
 * Before the {@code doInBackground} method is invoked on a <i>worker</i> thread, {@code SwingWorker} notifies any {@code PropertyChangeListeners} about the
 * {@code state} property change to {@code StateValue.STARTED}. After the {@code doInBackground} method is finished the {@code done} method is executed. Then
 * {@code SwingWorker} notifies any {@code PropertyChangeListeners} about the {@code state} property change to {@code StateValue.DONE}.
 * <p>
 * {@code SwingWorker} is only designed to be executed once. Executing a {@code SwingWorker} more than once will not result in invoking the {@code
 ...
 * <p>
 * Now we want to find the first N prime numbers and display the results in a {@code JTextArea}. While this is computing, we want to update our progress in a
 * {@code JProgressBar}. Finally, we also want to print the prime numbers to {@code System.out}.
 * 
 * <pre>
 ...
 * @version $Revision: 1.2 $ $Date: 2009/02/25 04:10:50 $
 * @param <T>
...
public abstract class SwingWorker<T, V> implements Future<T>, Runnable
{
	...
	/**
	 * Sends data chunks to the {@link #process} method. This method is to be used from inside the {@code doInBackground} method to deliver intermediate results
	 * for processing on the <i>Event Dispatch Thread</i> inside the {@code process} method.
	 * <p>
	 * Because the {@code process} method is invoked asynchronously on the <i>Event Dispatch Thread</i> multiple invocations to the {@code publish} method might
	 * occur before the {@code process} method is executed. For performance purposes all these invocations are coalesced into one invocation with concatenated
	 * arguments.
	 * <p>
	 * For example:
	 * 
	 ...
	 * </pre>
	 * <p>
	 * <b>Sample Usage</b>. This code snippet loads some tabular data and updates {@code DefaultTableModel} with it. Note that it safe to mutate the tableModel
	 * from inside the {@code process} method because it is invoked on the <i>Event Dispatch Thread</i>.
	 * 
	 * <pre>
	...
	/**
	 * Executed on the <i>Event Dispatch Thread</i> after the {@code doInBackground} method is finished. The default implementation does nothing. Subclasses may
	 * override this method to perform completion actions on the <i>Event Dispatch Thread</i>. Note that you can query status inside the implementation of this
	 * method to determine the result of this task or whether this task has been cancelled.
	 * 
	 * @see #doInBackground
	 * @see #isCancelled()
	 * @see #get
	 ...
	 * <p>
	 * Because {@code PropertyChangeListener}s are notified asynchronously on the <i>Event Dispatch Thread</i> multiple invocations to the {@code setProgress}
	 * method might occur before any {@code PropertyChangeListeners} are invoked. For performance purposes all these invocations are coalesced into one
	...
	/**
	 * Schedules this {@code SwingWorker} for execution on a <i>worker</i> thread. There are a number of <i>worker</i> threads available. In the event all
	 * <i>worker</i> threads are busy handling other {@code SwingWorkers} this {@code SwingWorker} is placed in a waiting queue.
	 * <p>
	 * Note: {@code SwingWorker} is only designed to be executed once. Executing a {@code SwingWorker} more than once will not result in invoking the {@code
	 * doInBackground} method twice.
	 */
	 ...
	 * {@inheritDoc}
	 * <p>
	 * Note: calling {@code get} on the <i>Event Dispatch Thread</i> blocks <i>all</i> events, including repaints, from being processed until this {@code
	...
	/**
	 * Adds a {@code PropertyChangeListener} to the listener list. The listener is registered for all properties. The same listener object may be added more
	 * than once, and will be called as many times as it is added. If {@code listener} is {@code null}, no exception is thrown and no action is taken.
	 * <p>
	...
	/**
	 * Removes a {@code PropertyChangeListener} from the listener list. This removes a {@code PropertyChangeListener} that was registered for all properties. If
	 * {@code listener} was added more than once to the same event source, it will be notified one less time after being removed. If {@code listener} is {@code
	 ...
	 * This {@code SwingWorker} will be the source for any generated events.
	 * <p>
	 * Note: The returned {@code PropertyChangeSupport} notifies any {@code PropertyChangeListener}s asynchronously on the <i>Event Dispatch Thread</i> in the
	 * event that {@code firePropertyChange} or {@code fireIndexedPropertyChange} are called off the <i>Event Dispatch Thread</i>.
	 * 
	...
	/**
	 * returns workersExecutorService. returns the service stored in the appContext or creates it if necessary. If the last one it triggers autoShutdown thread
	 * to get started.
	 * 
	...
	private static synchronized ExecutorService getWorkersExecutorService()
	{
		if (executorService == null)
		{
			...
			/*
			 * We want a to have no more than MAX_WORKER_THREADS running threads. We want a worker thread to wait no longer than 1 second for new tasks before
			 * terminating.
			 */
			executorService = new ThreadPoolExecutor(0, MAX_WORKER_THREADS, 5L, TimeUnit.SECONDS, new LinkedBlockingQueue<Runnable>(), threadFactory)
			{
				...
				public void execute(Runnable command)
				{
					 ...
					 * it run task in maximumPool. We want corePool to be 0 and maximumPool to be MAX_WORKER_THREADS We need to change the order of the
					 * execution. First try corePool then try maximumPool pool and only then store to the waiting queue. We can not do that because we would
					 * need access to the private methods. Instead we enlarge corePool to MAX_WORKER_THREADS before the execution and shrink it back to 0 after.
					 * It does pretty much what we need. While we changing the corePoolSize we need to stop running worker threads from accepting new tasks.
					 */
